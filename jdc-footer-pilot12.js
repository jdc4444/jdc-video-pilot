...
(function () {
  ...
  var projectSpacingQuery = new URLSearchParams(window.location.search).get("jdc-project-spacing");
  ...
  var projectSpacingEnabled = projectSpacingQuery !== "0" && window.location.pathname !== "/";
  ...
  function installProjectSpacing() {
    ...
    var leadMedia = lead.querySelector(".native-video-player") || leadShell;
    ...
    if (!document.getElementById("jdc-project-spacing-styles")) {
      ...
      style.textContent = [
        ...
        ".jdc-project-spacing .jdc-project-spacing-engine>.jdc-project-following-block{translate:0 calc(var(--jdc-project-flow-shift,0px) + var(--jdc-project-slot-shift,0px))!important}",
    ...
    function setFollowingSlotShifts(desktop) {
      trailing.forEach(function (element) { element.style.removeProperty("--jdc-project-slot-shift"); });
      if (desktop || !trailing.length) return 0;
      var bodyArea = gridArea(body);
      var followingAreas = trailing.map(function (element) {
        return { element: element, area: gridArea(element) };
      }).filter(function (item) { return item.area; });
      if (!bodyArea || !followingAreas.length) return 0;
      var firstFollowingStart = Math.min.apply(Math, followingAreas.map(function (item) { return item.area[0]; }));
      if (bodyArea[0] < firstFollowingStart) return 0;
      var rowSizes = getComputedStyle(engine).gridTemplateRows.split(" ").map(pixel);
      var rowGap = pixel(getComputedStyle(engine).rowGap);
      var slotHeight = rowSizes.slice(bodyArea[0] - 1, bodyArea[2] - 1).reduce(function (sum, value) {
        return sum + value;
      }, 0) + Math.max(0, bodyArea[2] - bodyArea[0]) * rowGap;
      if (!slotHeight) return 0;
      followingAreas.forEach(function (item) {
        if (item.area[0] >= bodyArea[2]) setPixel(item.element, "--jdc-project-slot-shift", -slotHeight);
      });
      return -slotHeight;
    }
    function schedule() {
      ...
      requestAnimationFrame(function () {
        ...
        setInfoGrid(desktop);
        var slotShift = setFollowingSlotShifts(desktop);
        ...
        var topDelta = headerRect.bottom + gap - leadRect.top;
        ...
        var currentFlowShift = pixel(getComputedStyle(engine).getPropertyValue("--jdc-project-flow-shift"));
        var firstFollowing = trailing.reduce(function (earliest, element) {
          var rect = element.getBoundingClientRect();
          return !earliest || rect.top < earliest.top ? rect : earliest;
        }, null);
        var settledInfoRect = infoBand.getBoundingClientRect();
        var nextFlowShift = 0;
        if (firstFollowing) {
          nextFlowShift = currentFlowShift + settledInfoRect.bottom + gap - firstFollowing.top;
        }
        setPixel(engine, "--jdc-project-flow-shift", nextFlowShift);
        setPixel(engine, "padding-bottom", Math.max(0, nextFlowShift));
        setPixel(engine, "margin-bottom", Math.min(0, nextFlowShift) + slotShift);
        ...
        document.body.setAttribute("data-jdc-project-flow-shift", Math.round(nextFlowShift * 100) / 100);
        pending = false;
        if (rerun) {
          rerun = false;
          schedule();
    ...
    if (window.ResizeObserver) {
      ...
      resizeObserver.observe(leadMedia);
  ...
  window.__JDC_SMART_VIDEO__ = {
    version: "adaptive-prefetch-pilot-12-responsive-project-info",
    states: states,
    activate: activate,
    unload: unload,
