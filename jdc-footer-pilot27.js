...
(function () {
  ...
  var BASIS_PROJECT_CLIPS = [
    { number: 1, duration: 30.037, poster: "https://jdc4444.github.io/jdc-video-pilot/media/basis/posters/basis-computer-first.jpg" },
    { number: 2, duration: 29.537, poster: "https://jdc4444.github.io/jdc-video-pilot/media/basis/posters/basis-phone-first.jpg" }
  ];
  var DIG_GALLERY_IMAGES = [
    { file: "01_aIFxYlGsbswqTMgP_digbowlanglewide.jpg", width: 2500, height: 1666 },
    { file: "02_aIFxY1GsbswqTMgQ_digposterw4.jpg", width: 1500, height: 1000 },
    { file: "03_aIGEYFGsbswqTMkv_Screen_Shot_2019-10-30_at_1.22.25_PM.webp.00_00_00_00.Still001.jpg", width: 1500, height: 1000 },
    { file: "04_aIFxZVGsbswqTMgS_digwindow1.webp.00_00_00_00.Still001.jpg", width: 1500, height: 1000 },
    { file: "05_aIFxZlGsbswqTMgT_digmenu.webp.00_00_00_00.Still001.jpg", width: 1500, height: 1000 },
    { file: "06_aIFxZ1GsbswqTMgU_Screen_Shot_2019-10-30_at_1.26.31_PM.webp.00_00_00_00.Still001.jpg", width: 1500, height: 1054 },
    { file: "07_aIGE4FGsbswqTMky_gingermint.webp.00_00_00_00.Still001.jpg", width: 1500, height: 1054 },
    { file: "08_aIGGQFGsbswqTMlB_Screen_Shot_2019-10-30_at_1.27.10_PM.webp.00_00_00_00.Still001.jpg", width: 1500, height: 791 },
    { file: "09_aIGGP1GsbswqTMlA_Screen_Shot_2019-10-30_at_1.23.28_PM.webp.00_00_00_00.Still001.jpg", width: 1500, height: 791 },
    { file: "10_aIGHSlGsbswqTMlP_icecream.jpg", width: 1500, height: 1125 },
    { file: "11_aIFxblGsbswqTMgb_IMG_0640.webp.00_00_00_00.Still001.jpg", width: 1500, height: 1125 },
    { file: "12_aIFxbFGsbswqTMgZ_Screen_Shot_2019-10-30_at_1.23.42_PM.webp.00_00_00_00.Still001.jpg", width: 1500, height: 843 },
    { file: "13_aIFxbVGsbswqTMga_Screen_Shot_2019-10-30_at_1.26.08_PM.webp.00_00_00_00.Still001.jpg", width: 1500, height: 861 },
    { file: "14_aIFxcFGsbswqTMgd_Screen_Shot_2019-10-30_at_1.24.13_PM.webp.00_00_00_00.Still001.jpg", width: 1500, height: 844 },
    { file: "15_aIFxb1GsbswqTMgc_Screen_Shot_2019-10-30_at_2.12.33_PM.webp.00_00_00_00.Still001.jpg", width: 1500, height: 841 },
    { file: "16_aIFxclGsbswqTMgf_Screen_Shot_2019-10-30_at_3.17.13_PM.webp.00_00_00_00.Still001.jpg", width: 1500, height: 999 },
    { file: "17_aIFxc1GsbswqTMgg_digcup.webp.00_00_00_00.Still001.jpg", width: 1500, height: 1000 },
    { file: "18_aIFxcVGsbswqTMge_Screen_Shot_2019-10-30_at_1.22.35_PM.webp.00_00_00_00.Still001.jpg", width: 1500, height: 842 },
    { file: "19_aIFxdFGsbswqTMgh_Screen_Shot_2019-10-30_at_1.24.27_PM.webp.00_00_00_00.Still001.jpg", width: 1500, height: 844 },
    { file: "20_aIFxdVGsbswqTMgi_Dig_Pledge.webp.00_00_00_00.Still001.jpg", width: 2350, height: 3216 }
  ];
  window.videoGlobalUnmute = false;
  ...
  function installBasisProject() {
    if (!/^\/basis\/?$/.test(window.location.pathname)) return false;
    ...
    document.querySelectorAll("[data-jdc-video]").forEach(function (shell) {
      ...
      var duration = Number(config && config.durationSeconds);
      if (!Number.isFinite(duration)) return;
      var match = BASIS_PROJECT_CLIPS.reduce(function (best, candidate) {
        var difference = Math.abs(duration - candidate.duration);
        return !best || difference < best.difference ? { clip: candidate, difference: difference } : best;
      }, null);
      if (!match || match.difference > 0.2 || clipsByNumber.has(match.clip.number)) return;
      clipsByNumber.set(match.clip.number, {
        shell: shell,
        number: match.clip.number,
        poster: match.clip.poster
    ...
    if (!BASIS_PROJECT_CLIPS.every(function (clip) { return clipsByNumber.has(clip.number); })) return false;

    var clips = Array.from(clipsByNumber.values()).sort(function (a, b) { return a.number - b.number; });
    var section = clips[0].shell.closest(".page-section, section");
    if (!section || !clips.every(function (clip) { return clip.shell.closest(".page-section, section") === section; })) return false;
    var title = Array.prototype.slice.call(section.querySelectorAll(".sqs-block-html h1")).find(function (heading) {
      return heading.textContent.trim().toLowerCase() === "basis";
    });
    if (!title) return false;

    clips.forEach(function (clip) {
      clip.shell.setAttribute("data-jdc-poster", clip.poster);
      clip.shell.setAttribute("data-jdc-basis-clip", String(clip.number));
    });

    var content = section.querySelector(":scope > .content-wrapper") || section.querySelector(".content-wrapper");
    if (!content) return false;
    var videoWrappers = clips.map(function (clip) { return clip.shell.closest(".fe-block") || clip.shell; });
    var titleWrapper = title.closest(".fe-block") || title.closest(".sqs-block-html") || title;
    var wrappers = [videoWrappers[0], titleWrapper, videoWrappers[1]];
    if (new Set(wrappers).size !== wrappers.length) return false;

    if (!document.getElementById("jdc-basis-project-styles")) {
      var style = document.createElement("style");
      ...
      style.textContent = [
        ...
        ".jdc-basis-project-grid>.jdc-basis-project-item>.sqs-block{box-sizing:border-box!important;width:100%!important;height:auto!important;min-height:0!important;padding:0!important}",
        ".jdc-basis-project-grid .sqs-block-content{height:auto!important;min-height:0!important}",
        ".jdc-basis-project-grid [data-jdc-video]{display:block!important;width:100%!important;height:auto!important;min-height:0!important;aspect-ratio:16/9!important;background-position:center!important;background-size:cover!important}",
        ".jdc-basis-project-grid>.jdc-basis-title{margin:clamp(18px,2vw,30px) 0!important}",
        ".jdc-basis-project-grid>.jdc-basis-title h1{margin:0!important}",
        ".jdc-basis-project-grid>.jdc-basis-phone{margin-top:0!important;margin-bottom:clamp(20px,2.2vw,34px)!important}",
        "@media(max-width:767px){.jdc-basis-project-grid>.jdc-basis-title{margin:14px 0!important}.jdc-basis-project-grid>.jdc-basis-phone{margin-bottom:14px!important}}"
      ].join("");
      document.head.appendChild(style);
    }

    var grid = content.querySelector(":scope > .jdc-basis-project-grid");
    if (!grid) {
      grid = document.createElement("div");
      grid.className = "jdc-basis-project-grid";
      content.appendChild(grid);
    }
    wrappers.forEach(function (wrapper, index) {
      wrapper.classList.add("jdc-basis-project-item");
    ...
    grid.setAttribute("data-jdc-basis-project-count", "2");
    section.classList.add("jdc-basis-project-section");
    section.setAttribute("data-jdc-basis-project", "ready");
    return true;
  }

  function installDigGallery() {
    if (!/^\/dig-brand-identity\/?$/.test(window.location.pathname)) return false;
    if (document.getElementById("jdc-dig-gallery-section")) return true;

    var article = document.querySelector("article.page-regions") || document.querySelector("main#page") || document.querySelector("main");
    if (!article) return false;
    var baseUrl = "https://jdc4444.github.io/jdc-video-pilot/media/dig/";

    if (!document.getElementById("jdc-dig-gallery-styles")) {
      var style = document.createElement("style");
      style.id = "jdc-dig-gallery-styles";
      style.textContent = [
        ".jdc-dig-gallery-section{box-sizing:border-box!important;width:100%!important;padding:clamp(8px,1vw,14px)!important;margin:0!important}",
        ".jdc-dig-gallery-grid{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:clamp(8px,1vw,14px)!important;width:100%!important;margin:0!important;padding:0!important}",
        ".jdc-dig-gallery-item{display:block!important;min-width:0!important;margin:0!important;background:#f2f2f2!important;overflow:hidden!important}",
        ".jdc-dig-gallery-item:first-child,.jdc-dig-gallery-item:last-child{grid-column:1/-1!important}",
        ".jdc-dig-gallery-item picture{display:block!important;width:100%!important}",
        ".jdc-dig-gallery-item img{display:block!important;width:100%!important;height:auto!important;margin:0!important;object-fit:contain!important;color:transparent!important}",
        "@media(max-width:767px){.jdc-dig-gallery-section{padding:8px!important}.jdc-dig-gallery-grid{grid-template-columns:minmax(0,1fr)!important;gap:8px!important}.jdc-dig-gallery-item{grid-column:1!important}}"
      ].join("");
      document.head.appendChild(style);
    }

    var section = document.createElement("section");
    section.id = "jdc-dig-gallery-section";
    section.className = "jdc-dig-gallery-section";
    section.setAttribute("data-jdc-dig-gallery", "ready");
    section.setAttribute("data-jdc-dig-gallery-count", String(DIG_GALLERY_IMAGES.length));
    section.setAttribute("aria-label", "DIG brand identity gallery");

    var grid = document.createElement("div");
    grid.className = "jdc-dig-gallery-grid";
    DIG_GALLERY_IMAGES.forEach(function (item, index) {
      var figure = document.createElement("figure");
      figure.className = "jdc-dig-gallery-item";
      figure.setAttribute("data-jdc-dig-order", String(index + 1));
      var webpName = item.file.replace(/\.jpg$/i, ".webp");
      var picture = document.createElement("picture");
      var source = document.createElement("source");
      source.type = "image/webp";
      source.srcset = baseUrl + "webp/960/" + encodeURIComponent(webpName) + " 960w, " + baseUrl + "webp/1600/" + encodeURIComponent(webpName) + " 1600w";
      source.sizes = index === 0 || index === DIG_GALLERY_IMAGES.length - 1 ? "100vw" : "(max-width: 767px) 100vw, 50vw";
      var image = document.createElement("img");
      image.src = baseUrl + "source/" + encodeURIComponent(item.file);
      image.width = item.width;
      image.height = item.height;
      image.alt = "DIG brand identity case study image " + String(index + 1) + " of " + String(DIG_GALLERY_IMAGES.length);
  ...
  window.__JDC_SMART_VIDEO__ = {
    version: "adaptive-prefetch-pilot-27-dig-gallery-basis-project-polymarket-seven-limn-gallery-inline",
    states: states,
    activate: activate,
    unload: unload,
