...
(function () {
  ...
  function installProjectSpacing() {
    ...
    function placeBombasGallery(gap) {
      if (!bombasProject || !introSection || !gallerySection || !gallerySection.querySelector(".sqs-block-video")) return null;
      ...
      var shift = textBottom + gap - firstGalleryMedia.getBoundingClientRect().top;
      setPixel(introSection, "margin-bottom", Math.min(0, shift));
      var actualGap = firstGalleryMedia.getBoundingClientRect().top - textBottom;
      return { shift: Math.min(0, shift), gap: actualGap };
    }
    function schedule() {
      if (pending) {
        rerun = true;
  ...
  window.__JDC_SMART_VIDEO__ = {
    version: "adaptive-prefetch-pilot-19-bombas-gallery-spacing",
    states: states,
    activate: activate,
    unload: unload,
