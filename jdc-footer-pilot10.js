...
(function () {
  ...
  function installProjectSpacing() {
    ...
    var leadMedia = lead.querySelector(".native-video-player") || lead.querySelector(".sqs-block-video") || lead;
    ...
    function schedule() {
      ...
      requestAnimationFrame(function () {
        pending = false;
        title.style.setProperty("--jdc-project-spacing-shift", "0px");
        body.style.setProperty("--jdc-project-spacing-shift", "0px");
        requestAnimationFrame(function () {
          var desktop = window.matchMedia("(min-width:768px)").matches;
          var gap = desktop ? Math.min(52, Math.max(32, window.innerWidth * 0.03)) : 24;
          var leadBottom = leadMedia.getBoundingClientRect().bottom;
          var titleRect = title.getBoundingClientRect();
          var bodyRect = body.getBoundingClientRect();
          var targetTop = leadBottom + gap;
          var titleShift = targetTop - titleRect.top;
          var bodyShift;
          if (desktop) {
            bodyShift = targetTop - bodyRect.top;
          } else {
            var titleBottom = titleRect.bottom + titleShift;
            bodyShift = Math.max(0, titleBottom + 16 - bodyRect.top);
          }
          title.style.setProperty("--jdc-project-spacing-shift", Math.round(titleShift * 100) / 100 + "px");
          body.style.setProperty("--jdc-project-spacing-shift", Math.round(bodyShift * 100) / 100 + "px");
  ...
  window.__JDC_SMART_VIDEO__ = {
    version: "adaptive-prefetch-pilot-10-project-spacing",
    states: states,
    activate: activate,
    unload: unload,
