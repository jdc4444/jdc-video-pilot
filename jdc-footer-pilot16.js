...
(function () {
  ...
  function publishHoverAudio() {
    var mode = hoverAudioPointer.matches ? (hoverAudioEnabled ? "enabled" : "awaiting-gesture") : "click-only";
  ...
  function mountVideo(state) {
    ...
    state.video = video;
    if (state.unmuteOnMount || (state.kind === "block" && state.hovered && hoverAudioEnabled && hoverAudioPointer.matches)) {
  ...
  function activate(state, userInitiated) {
    ...
    if (nativeAppleHls) {
      state.shell.setAttribute("data-jdc-start-quality", "native-adaptive");
      video.src = source;
      state.loading = false;
    ...
    ensureHls().then(function (Hls) {
      ...
      var hls = new Hls({
        /* Dense galleries always begin at the lightest rendition, then ABR can rise in place. */
        startLevel: denseGallery ? 0 : turbo ? (state.usingPilot ? 2 : 1) : conservative ? 0 : -1,
        startFragPrefetch: true,
        capLevelToPlayerSize: true,
        capLevelOnFPSDrop: true,
        maxBufferLength: denseGallery ? (conservative ? 6 : 10) : conservative ? 8 : turbo ? 30 : 15,
        maxMaxBufferLength: denseGallery ? (conservative ? 10 : 18) : conservative ? 12 : turbo ? 60 : 25,
        backBufferLength: 0
      });
      state.shell.setAttribute("data-jdc-start-quality", denseGallery ? "lowest-first" : "adaptive");
      state.hls = hls;
  ...
  function register(shell, config, kind) {
    ...
    var state = { shell: shell, stage: stage, config: config, kind: kind, order: states.size, video: null, hls: null, loading: false, starting: false, startupTimer: null, startupOutcome: "poster", controls: null, retryCount: 0, retryTimer: null, pilotFailed: false, usingPilot: false, hovered: false, unmuteOnMount: false };
    ...
    if (kind === "block") {
      ...
      shell.addEventListener("mouseenter", function () {
        state.hovered = true;
        if (hoverAudioEnabled && hoverAudioPointer.matches) unmuteState(state);
      });
  ...
  window.__JDC_SMART_VIDEO__ = {
    version: "adaptive-prefetch-pilot-16-hover-audio",
    states: states,
    activate: activate,
    unload: unload,
